...
Matrix createLocalK(int e,mesh &m){
    ...
    float const_t,const_k,delta,lambda,Ae,J,D;
    
    //Componentes de K
    // [ A+K  G ]
    // [  D   0 ]
    Matrix matrixB,matrixK,matrixG,matrixL,matrixE;
    Matrix K,g_matrix,g_matrix_t,Alpha,Beta,Alphat,Betat,BPrima,BPrimat;

    //Preparando matrixB
    const_t = m.getParameter(CONST_T);
...
Vector createLocalb(int e,mesh &m){
    ...
    float psi_x = m.getParameter(PSI_IN_X);
    float psi_y = m.getParameter(PSI_IN_Y);
